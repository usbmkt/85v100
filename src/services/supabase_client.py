    ...
    def _validate_api_key(self) -> bool:
        ...
        try:
            ...
            if 'invalid api key' in error_str or 'unauthorized' in error_str:
                logger.warning("⚠️ Chave de API do Supabase inválida - usando apenas arquivos locais")
                self.enabled = False
                return False
    ...
    def _insert_with_retry(self, insert_data: Dict[str, Any], max_retries: int = 3) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'invalid api key' in error_str or 'unauthorized' in error_str:
                    # Erro de autenticação - não tenta novamente e desabilita o cliente
                    logger.warning("⚠️ Chave de API do Supabase inválida durante inserção - usando apenas arquivos locais")
                    self.enabled = False
                    raise Exception("Supabase API key inválida") from e
