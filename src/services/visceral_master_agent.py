...
from services.auto_save_manager import salvar_etapa, salvar_erro, auto_save_manager
...
class VisceralMasterAgent:
    ...
    def __init__(self):
        """Inicializa o mestre visceral"""
        self.ai_manager = ai_manager
        logger.info("Visceral Master Agent inicializado")

    def generate_visceral_persuasion(self, context_data: Dict[str, Any], avatar_data: Dict[str, Any]) -> Dict[str, Any]:
        """Gera persuasão visceral completa"""
        try:
            logger.info("🔥 Gerando persuasão visceral")

            # Persuasão visceral básica
            persuasion_elements = {
                "gatilhos_emocionais": [
                ...
                "narrativas_impactantes": [
                    "História de transformação real",
                    "Jornada do herói personalizada",
                    "Conflito e resolução",
                    "Futuro desejado vs presente atual"
                ],
                "calls_to_action": [
            ...
            return {
                "status": "sucesso",
                "persuasion_elements": persuasion_elements,
                "intensidade": "alta"
            }

        except Exception as e:
            logger.error(f"❌ Erro na persuasão visceral: {e}")
            salvar_erro("visceral_falha", e, {"context": context_data})
            return {
                "status": "erro",
                "persuasion_elements": {},
                "intensidade": "baixa"
...
# Instância global
visceral_master_agent = VisceralMasterAgent()
    ...
    def execute_visceral_analysis(
        ...
        try:
            ...
            if not response or not isinstance(response, str):
                raise Exception("MESTRE VISCERAL FALHOU: IA não respondeu adequadamente")
