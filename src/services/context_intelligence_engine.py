...
class ContextIntelligenceEngine:
    ...
    def _enrich_with_ai(self, context: Dict[str, Any], segmento: str, produto: str = None) -> Dict[str, Any]:
        """Enriquece o contexto com inteligência artificial ESPECÍFICA"""
        
        # Analisa o segmento específico para personalizar a análise
        segment_keywords = self._extract_segment_keywords(segmento)
        product_keywords = self._extract_product_keywords(produto) if produto else []
        
        prompt = f"""
        ANÁLISE ULTRA-ESPECÍFICA DE MERCADO - NÃO USE PLACEHOLDERS GENÉRICOS!

        SEGMENTO ESPECÍFICO: {segmento}
        PRODUTO ESPECÍFICO: {produto or "Não especificado"}
        PALAVRAS-CHAVE DO SEGMENTO: {', '.join(segment_keywords)}
        PALAVRAS-CHAVE DO PRODUTO: {', '.join(product_keywords)}
        
        CONTEXTO ATUAL: {json.dumps(context, indent=2, ensure_ascii=False)}
        
        IMPORTANTE: Baseie-se EXCLUSIVAMENTE nas palavras-chave e contexto específicos fornecidos.
        NÃO use termos genéricos como "dash", "conceito", "eficácia" sem contexto real.
        
        Para o segmento "{segmento}" e produto "{produto}", forneça insights REAIS e ESPECÍFICOS:

        1. DORES ESPECÍFICAS do público que compra exatamente este tipo de produto/serviço
        2. DESEJOS CONCRETOS relacionados especificamente ao {segmento}
        3. OBJEÇÕES REAIS que este público específico tem
        4. CONCEITOS ESPECÍFICOS que precisam ser provados (relacionados ao produto real)
        5. GATILHOS PSICOLÓGICOS que funcionam especificamente neste nicho
        6. LINGUAGEM e VOCABULÁRIO que este público específico usa
        7. MÉTRICAS DE SUCESSO específicas do {segmento}
        8. TRANSFORMAÇÕES específicas prometidas por produtos como {produto}
        9. TIMELINE REALISTA para ver resultados neste segmento específico
        10. FATORES DE CREDIBILIDADE importantes especificamente para {segmento}

        Responda APENAS com um JSON válido usando o contexto REAL fornecido:
        {{
            "specific_pain_points": ["dor específica 1", "dor específica 2", "..."],
            "deep_desires": ["desejo específico 1", "desejo específico 2", "..."],
            "real_objections": ["objeção real 1", "objeção real 2", "..."],
            "proof_concepts": ["conceito específico a provar 1", "conceito específico a provar 2", "..."],
            "psychological_triggers": ["gatilho específico 1", "gatilho específico 2", "..."],
            "typical_language": ["termo usado 1", "expressão típica 2", "..."],
            "success_metrics": ["métrica específica 1", "métrica específica 2", "..."],
            "transformation_promises": ["transformação específica 1", "transformação específica 2", "..."],
            "realistic_timeline": "timeline específico para {segmento}",
            "credibility_factors": ["fator credibilidade 1", "fator credibilidade 2", "..."]
        }}
        """
        
        try:
            ai_response = ai_manager.generate_analysis(prompt)
    ...
    def _extract_segment_keywords(self, segmento: str) -> List[str]:
        """Extrai palavras-chave específicas do segmento"""
        words = segmento.lower().split()
        # Remove palavras muito comuns
        stop_words = {'de', 'da', 'do', 'das', 'dos', 'em', 'na', 'no', 'para', 'com', 'por', 'um', 'uma', 'o', 'a', 'os', 'as'}
        keywords = [word for word in words if word not in stop_words and len(word) > 2]
        return keywords[:5]  # Máximo 5 palavras-chave
    
    def _extract_product_keywords(self, produto: str) -> List[str]:
        """Extrai palavras-chave específicas do produto"""
        if not produto:
            return []
        words = produto.lower().split()
        stop_words = {'de', 'da', 'do', 'das', 'dos', 'em', 'na', 'no', 'para', 'com', 'por', 'um', 'uma', 'o', 'a', 'os', 'as'}
        keywords = [word for word in words if word not in stop_words and len(word) > 2]
        return keywords[:5]  # Máximo 5 palavras-chave

# Instância global
context_intelligence_engine = ContextIntelligenceEngine()
